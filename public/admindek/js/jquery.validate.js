...
*/(function (factory) { if (typeof define === "function" && define.amd) { define(["jquery"], factory); } else if (typeof module === "object" && module.exports) { module.exports = factory(require("jquery")); } else { factory(jQuery); } }(function ($) {
    $.extend($.fn, {
        ...
        defaults: {
        ...
        }, setDefaults: function (settings) { $.extend($.validator.defaults, settings); }, messages: { 
            ...
            maxlength: $.validator.format("โปรดกรอกให้ครบ {0} ตัว."), 
            minlength: $.validator.format("โปรดกรอกให้ครบ {0} ตัว."), 
            ...
            init: function () {
                ...
                if (element.name === "hn"){
                    ...
                    $.validator.messages[rule.method], "<strong style='color:red;'>** กรุณากรอกรหัส HN **" 
                    + "</strong>"), theregex = /\$?\{(\d+)\}/g; 
                } else if(element.name === "fname"){
                var message = this.findDefined(this.customMessage(element.name, rule.method), 
                            this.customDataMessage(element, rule.method), 
                            !this.settings.ignoreTitle && element.title || undefined, 
                            $.validator.messages[rule.method], "<strong style='color:red;'>** กรุณากรอกชื่อ **" 
                            + "</strong>"), theregex = /\$?\{(\d+)\}/g; 
                } else if(element.name === "lname"){
                    var message = this.findDefined(this.customMessage(element.name, rule.method), 
                                this.customDataMessage(element, rule.method), 
                                !this.settings.ignoreTitle && element.title || undefined, 
                                $.validator.messages[rule.method], "<strong style='color:red;'>** กรุณากรอกนามสกุล **" 
                                + "</strong>"), theregex = /\$?\{(\d+)\}/g; 
                } else if(element.name === "pa_phone"){
                    var message = this.findDefined(this.customMessage(element.name, rule.method), 
                                this.customDataMessage(element, rule.method), 
                                !this.settings.ignoreTitle && element.title || undefined, 
                                $.validator.messages[rule.method], "<strong style='color:red;'>** กรุณากรอกเบอร์โทรศัพท์ผู้ป่วย **" 
                                + "</strong>"), theregex = /\$?\{(\d+)\}/g; 
                } else if(element.name === "pa_sex"){
                    var message = this.findDefined(this.customMessage(element.name, rule.method), 
                                this.customDataMessage(element, rule.method), 
                                !this.settings.ignoreTitle && element.title || undefined, 
                                $.validator.messages[rule.method], "<strong style='color:red;'>** กรุณาเลือกเพศ **" 
                                + "</strong>"), theregex = /\$?\{(\d+)\}/g; 
                } else if(element.name === "pay"){
                    var message = this.findDefined(this.customMessage(element.name, rule.method), 
                                this.customDataMessage(element, rule.method), 
                                !this.settings.ignoreTitle && element.title || undefined, 
                                $.validator.messages[rule.method], "<strong style='color:red;'>** กรุณาเลือกสิทธิ์การรักษา **" 
                                + "</strong>"), theregex = /\$?\{(\d+)\}/g; 
                } else if(element.name === "disease"){
                    var message = this.findDefined(this.customMessage(element.name, rule.method), 
                                this.customDataMessage(element, rule.method), 
                                !this.settings.ignoreTitle && element.title || undefined, 
                                $.validator.messages[rule.method], "<strong style='color:red;'>** กรุณากรอกวินิจฉัยโรค **" 
                                + "</strong>"), theregex = /\$?\{(\d+)\}/g; 
                } else if(element.name === "opt_id"){
                    var message = this.findDefined(this.customMessage(element.name, rule.method), 
                                this.customDataMessage(element, rule.method), 
                                !this.settings.ignoreTitle && element.title || undefined, 
                                $.validator.messages[rule.method], "<strong style='color:red;'>** กรุณาเลือกหัตถการ **" 
                                + "</strong>"), theregex = /\$?\{(\d+)\}/g; 
                } else if(element.name === "ward"){
                    var message = this.findDefined(this.customMessage(element.name, rule.method), 
                                this.customDataMessage(element, rule.method), 
                                !this.settings.ignoreTitle && element.title || undefined, 
                    ...
                    var message = this.findDefined(this.customMessage(element.name, rule.method), 
                                this.customDataMessage(element, rule.method), 
                                !this.settings.ignoreTitle && element.title || undefined, 
                                $.validator.messages[rule.method], "<strong style='color:red;'>** กรุณาเลือกอาจารย์แพทย์ **" 
                                + "</strong>"), theregex = /\$?\{(\d+)\}/g; 
                } else if(element.name === "pa_age"){
                    var message = this.findDefined(this.customMessage(element.name, rule.method), 
                                this.customDataMessage(element, rule.method), 
                                !this.settings.ignoreTitle && element.title || undefined, 
                    ...
                    var message = this.findDefined(this.customMessage(element.name, rule.method), 
                                this.customDataMessage(element, rule.method), 
                                !this.settings.ignoreTitle && element.title || undefined, 
                                $.validator.messages[rule.method], "<strong style='color:red;'>** กรุณาเลือกวันที่ต้องการจองเตียง **" 
                                + "</strong>"), theregex = /\$?\{(\d+)\}/g; 
                } else if(element.name === "prefix"){
                    var message = this.findDefined(this.customMessage(element.name, rule.method), 
                                this.customDataMessage(element, rule.method), 
                                !this.settings.ignoreTitle && element.title || undefined, 
                                $.validator.messages[rule.method], "<strong style='color:red;'>** เลือกคำนำหน้า **" 
                                + "</strong>"), theregex = /\$?\{(\d+)\}/g; 
                } else {
                    var message = this.findDefined(this.customMessage(element.name, rule.method), 
                            this.customDataMessage(element, rule.method), 
                            !this.settings.ignoreTitle && element.title || undefined, 
                            $.validator.messages[rule.method], "<strong style='color:red;'>** กรุณากรอกหรือเลือก ในช่องนี้ **" 
                            + "</strong>"), theregex = /\$?\{(\d+)\}/g; 
                }
